...
class CSharpFunctionProjectParser extends FunctionProjectCodeParser {
    ...
    async traverseProjectCode(projectFolder) {
        ...
        for await (const func of this._fileSystemWrapper.findFunctionsRecursivelyAsync(projectFolder, fileNameRegex, traverseFunctionProjectUtils_1.BindingsParser.functionAttributeRegex, 3)) {
...
class FSharpFunctionProjectParser extends FunctionProjectCodeParser {
    ...
    async traverseProjectCode(projectFolder) {
        const result = {};
        for await (const func of this._fileSystemWrapper.findFunctionsRecursivelyAsync(projectFolder, new RegExp('.+\\.fs$', 'i'), traverseFunctionProjectUtils_1.BindingsParser.fSharpFunctionAttributeRegex, 2)) {
            const bindings = traverseFunctionProjectUtils_1.BindingsParser.tryExtractBindings(func.declarationCode);
            result[func.functionName] = {
                filePath: func.filePath,
                pos: func.pos,
...
class JavaFunctionProjectParser extends FunctionProjectCodeParser {
    ...
    async traverseProjectCode(projectFolder) {
        const result = {};
        for await (const func of this._fileSystemWrapper.findFunctionsRecursivelyAsync(projectFolder, new RegExp('.+\\.java$', 'i'), traverseFunctionProjectUtils_1.BindingsParser.javaFunctionAttributeRegex, 1)) {
            const bindings = traverseFunctionProjectUtils_1.BindingsParser.tryExtractBindings(func.declarationCode);
            result[func.functionName] = {
                filePath: func.filePath,
                pos: func.pos,
    ...
    context.subscriptions.push(vscode.commands.registerCommand('az-func-as-a-graph.ShowGraph', async (item) => {
        ...
        for (const folder of vscode.workspace.workspaceFolders) {
            ...
            if (await fsWrapper.pathExists(hostJsonPath)) {
                graphViews.push(new FunctionGraphView_1.FunctionGraphView(context, folder.uri));
                hostJsonFound = true;
            }
        }
        if (!hostJsonFound && !!vscode.workspace.workspaceFolders.length) {
            ...
            graphViews.push(new FunctionGraphView_1.FunctionGraphView(context, vscode.workspace.workspaceFolders[0].uri));
        }
    }));
    if (!vscode.workspace.workspaceFolders) {
        return;
